...
from graphlit_helper import query_graphlit, query_graphlit_web
import asyncio
        ...
        if thread.name.lower() == expected_name and not thread.archived:
            try:
                async with thread.typing():
                    answer = await query_graphlit_web(
...
async def ask(ctx, *, question: str = None):
    try:
        ...
        async with ctx.typing():
            answer = await query_graphlit(str(ctx.guild.id), question)
            if is_no_kb_response(answer):
                answer = await query_graphlit_web(str(ctx.guild.id), question)
        await send_answer_with_feedback(
...
async def help(ctx):
    ...
    embed.add_field(
        name="`-ask <question>`",
        value="Ask a question from the knowledge base.\nExample: `-ask What is Python?`",
        inline=False
    )
    embed.add_field(
        name="`#support` channel",
        value="Send any message in `#support` to open a private ticket thread and get an answer.",
        inline=False
    )
    embed.add_field(
        name="`-close`",
        value="Close and archive your ticket thread. Run this inside the thread.",
        inline=False
    )
