...
function LandingPage({ user, onLogin, onShowDashboard }) {
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <nav style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 40px",height:64,background:"rgba(255,248,245,0.85)",backdropFilter:"blur(16px)",borderBottom:"1px solid rgba(199,196,215,0.3)",position:"sticky",top:0,zIndex:50 }}>
        <div style={{ display:"flex",alignItems:"center",gap:12 }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          {user ? <Btn onClick={onShowDashboard}><Icon name="dashboard" size={16}/> Open Dashboard</Btn>
                : <Btn onClick={onLogin} variant="discord"><DiscordIcon/> Login with Discord</Btn>}
        </div>
      </nav>

      <section style={{ padding:"96px 40px 80px",textAlign:"center",maxWidth:900,margin:"0 auto",position:"relative",overflow:"hidden" }}>
        ...
        <div style={{ position:"relative" }} className="au">
          ...
          <h1 style={{ fontSize:"clamp(36px,6vw,60px)",fontWeight:800,lineHeight:1.05,letterSpacing:"-0.03em",marginBottom:20 }}>
            Give Your Discord Server<br/><span style={{ color:"var(--primary)" }}>an AI Brain</span>
          </h1>
          ...
          <div style={{ display:"flex",justifyContent:"center",gap:12,flexWrap:"wrap" }}>
            {user ? <Btn onClick={onShowDashboard} style={{ fontSize:15,padding:"13px 28px" }}><Icon name="dashboard"/> Open Dashboard</Btn>
                  : <><Btn onClick={onLogin} variant="discord" style={{ fontSize:15,padding:"13px 28px" }}><DiscordIcon size={20}/> Get Started with Discord</Btn>
                     <Btn onClick={onShowDashboard} variant="ghost" style={{ fontSize:14 }}>Preview Dashboard</Btn></>}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding:"32px 40px",borderTop:"1px solid rgba(199,196,215,0.3)",borderBottom:"1px solid rgba(199,196,215,0.3)",background:"var(--surface-lowest)" }}>
        <div style={{ maxWidth:800,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:24,textAlign:"center" }}>
          {STATS.map((s,i) => <div key={i}><div style={{ fontSize:36,fontWeight:800,color:"var(--primary)",letterSpacing:"-0.03em" }}>{s.value}</div><div style={{ fontSize:13,color:"var(--on-surface-variant)",marginTop:4 }}>{s.label}</div></div>)}
        </div>
      </section>

      <section style={{ padding:"72px 40px",background:"var(--surface-low)",borderTop:"1px solid rgba(199,196,215,0.3)" }}>
        <div style={{ maxWidth:1100,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:48 }}><Tag style={{ marginBottom:12 }}>Capabilities</Tag><h2 style={{ fontSize:32,fontWeight:700,letterSpacing:"-0.02em" }}>Everything Your Bot Can Do</h2></div>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:16 }}>
            {FEATURES.map((f,i) => <Card key={i}><div style={{ width:44,height:44,borderRadius:"var(--r-md)",background:"var(--primary-fixed)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:14 }}><Icon name={f.icon} size={22} style={{ color:"var(--primary)" }}/></div><div style={{ fontSize:14,fontWeight:600,marginBottom:6 }}>{f.title}</div><div style={{ fontSize:13,color:"var(--on-surface-variant)",lineHeight:1.6 }}>{f.desc}</div></Card>)}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding:"72px 40px" }}>
        <div style={{ maxWidth:1100,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:48 }}><Tag style={{ marginBottom:12 }}>Setup</Tag><h2 style={{ fontSize:32,fontWeight:700,letterSpacing:"-0.02em" }}>Up and Running in Minutes</h2></div>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:16 }}>
            {HOW_IT_WORKS.map((s,i) => <Card key={i}><div style={{ fontSize:11,fontWeight:700,letterSpacing:"0.08em",color:"var(--primary)",opacity:0.4,marginBottom:10 }}>STEP {s.step}</div><div style={{ width:44,height:44,borderRadius:"var(--r-md)",background:"var(--primary-fixed)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:14 }}><Icon name={s.icon} size={22} style={{ color:"var(--primary)" }}/></div><div style={{ fontSize:14,fontWeight:600,marginBottom:6 }}>{s.title}</div><div style={{ fontSize:13,color:"var(--on-surface-variant)",lineHeight:1.6 }}>{s.desc}</div></Card>)}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding:"72px 40px",background:"var(--surface-low)",borderTop:"1px solid rgba(199,196,215,0.3)" }}>
        <div style={{ maxWidth:1000,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:48 }}><Tag style={{ marginBottom:12 }}>Why VaultBot</Tag><h2 style={{ fontSize:32,fontWeight:700,letterSpacing:"-0.02em" }}>How We Stack Up</h2></div>
          <Card pad="0" style={{ overflow:"hidden" }}>
            <div style={{ overflowX:"auto" }}>
              <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13.5 }}>
                <thead>
                  <tr>
                    <th style={{ padding:"14px 20px",textAlign:"left",background:"var(--surface-low)",borderBottom:"1px solid rgba(199,196,215,0.3)",fontSize:12,fontWeight:700,letterSpacing:"0.05em",textTransform:"uppercase",color:"var(--on-surface-variant)",minWidth:200 }}>Feature</th>
                    <th style={{ padding:"14px 20px",textAlign:"center",background:"rgba(70,72,212,0.06)",borderBottom:"1px solid rgba(199,196,215,0.3)",fontSize:13,fontWeight:700,color:"var(--primary)",minWidth:120,borderLeft:"2px solid var(--primary)" }}>VaultBot</th>
                    {["MEE6","Carl-bot","Atlas"].map(n => <th key={n} style={{ padding:"14px 20px",textAlign:"center",background:"var(--surface-low)",borderBottom:"1px solid rgba(199,196,215,0.3)",fontSize:12,fontWeight:700,letterSpacing:"0.05em",textTransform:"uppercase",color:"var(--on-surface-variant)",minWidth:110 }}>{n}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {COMPARISON.map((row,i) => <tr key={i}><td style={{ padding:"13px 20px",fontWeight:500,color:"var(--on-surface)",borderBottom:"1px solid rgba(199,196,215,0.15)",fontSize:13.5 }}>{row.feature}</td><CmpCell val={row.yours} highlight/><CmpCell val={row.mee6}/><CmpCell val={row.carl}/><CmpCell val={row.atlas}/></tr>)}
                </tbody>
              </table>
            </div>
          </Card>
      ...
      <section style={{ padding:"72px 40px" }}>
        <div style={{ maxWidth:720,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:48 }}><Tag style={{ marginBottom:12 }}>FAQ</Tag><h2 style={{ fontSize:32,fontWeight:700,letterSpacing:"-0.02em" }}>Common Questions</h2></div>
          <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
            {FAQS.map((f,i) => <Card key={i}><div style={{ fontSize:14,fontWeight:600,marginBottom:8,display:"flex",alignItems:"flex-start",gap:10 }}><Icon name="help" size={18} style={{ color:"var(--primary)",flexShrink:0,marginTop:1 }}/>{f.q}</div><div style={{ fontSize:13,color:"var(--on-surface-variant)",lineHeight:1.7,paddingLeft:28 }}>{f.a}</div></Card>)}
          </div>
        </div>
      </section>
