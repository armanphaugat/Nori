...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --navy-mid:    #1a5570;
    --steel:       #598392;
    --steel-dim:   rgba(89,131,146,0.18);
    --steel-glow:  rgba(89,131,146,0.32);
    --sage:        #AEC3B0;
    --sage-dim:    rgba(174,195,176,0.14);
    --cream:       #EFF6E0;
    --cream-dim:   rgba(239,246,224,0.08);

    --accent:      var(--steel);
    --accent-light: #7fb3c3;
    --accent-glow:  rgba(89,131,146,0.28);
    --border:       rgba(89,131,146,0.12);
    --border2:      rgba(89,131,146,0.35);
    --text:         var(--cream);
    --muted:        var(--sage);
    --muted2:       rgba(174,195,176,0.55);
  ...
  ::-webkit-scrollbar-thumb { background: #124559; border-radius: 99px; }

  .glass {
    background: rgba(9,26,38,0.65);
    backdrop-filter: blur(16px);
    border: 1px solid var(--border);
  }
  .glass-hover:hover {
    background: rgba(12,32,48,0.85) !important;
  ...
  .water-bg {
    ...
    background-image: 
      repeating-linear-gradient(
        ...
        transparent,
        transparent 2px,
        rgba(89,131,146,0.15) 2px,
        rgba(89,131,146,0.15) 4px
      );
  ...
  .bloom {
    ...
    background: radial-gradient(circle, rgba(89,131,146,0.1) 0%, transparent 70%);
  }
  .bloom-sage {
    position: absolute; pointer-events: none; z-index: 0;
    background: radial-gradient(circle, rgba(174,195,176,0.07) 0%, transparent 70%);
  }
  ...
  @keyframes blink { 0%,100%{box-shadow:0 0 0 0 rgba(89,131,146,0.5);} 70%{box-shadow:0 0 0 8px transparent;} }
  @keyframes glow-pulse { 0%,100%{box-shadow:0 0 22px rgba(89,131,146,0.2);} 50%{box-shadow:0 0 40px rgba(89,131,146,0.38);} }
  ...
  .feat-card:hover{background:rgba(12,32,48,0.95)!important;transform:translateY(-3px);box-shadow:0 20px 40px rgba(0,0,0,0.4);border-color:var(--border2)!important;}
  .feat-card:hover .feat-icon{box-shadow:0 0 18px var(--steel-glow);}
  .step-wrap:hover .step-icon{border-color:var(--border2)!important;box-shadow:0 0 22px var(--steel-glow)!important;}
  .use-card:hover{border-color:var(--border2)!important;transform:translateY(-3px);}
  .nav-link:hover{color:var(--cream);background:rgba(89,131,146,0.08);}
  .btn-primary:hover{opacity:0.88;transform:translateY(-1px);box-shadow:0 0 36px var(--steel-glow)!important;}
  .btn-ghost:hover{background:rgba(89,131,146,0.08)!important;color:var(--cream)!important;}
  .btn-sm:hover{background:var(--navy-mid)!important;box-shadow:0 0 22px var(--steel-glow)!important;}

  .glow-ocean{animation:glow-pulse 3s ease infinite;}
  ...
  /* Ocean shimmer text */
  .shimmer-text {
    background: linear-gradient(90deg, var(--steel) 0%, var(--accent-light) 30%, var(--sage) 50%, var(--accent-light) 70%, var(--steel) 100%);
  ...
  .hero-line {
    ...
    background: linear-gradient(to bottom, transparent, rgba(89,131,146,0.2), transparent);
    pointer-events: none;
  }
...
const FEATS = [
  { icon:"robot_2",        fill:1, color:"steel",  title:"Your Own AI Discord Bot",      desc:"Create a custom AI chatbot for your Discord server trained on your own documents and websites." },
  { icon:"picture_as_pdf", fill:1, color:"sage",   title:"Upload PDFs & Files",          desc:"Upload PDFs, notes, Excel sheets, or text files and let the AI learn from them instantly." },
  { icon:"language",       fill:1, color:"steel",  title:"Website Ingestion & Crawler",  desc:"Import any website URL or auto-crawl entire documentation folders in one click." },
  { icon:"question_answer",fill:1, color:"sage",   title:"Answers From Your Data",       desc:"The bot answers questions using your uploaded content instead of random internet guesses." },
  { icon:"tune",           fill:1, color:"steel",  title:"Custom Bot Personality",       desc:"Change how the bot talks, behaves, and responds with your own custom system prompt instructions." },
  { icon:"forum",          fill:1, color:"sage",   title:"Choose Bot Channels",          desc:"Select exactly which Discord channels the bot can read and reply in." },
  { icon:"bar_chart",      fill:1, color:"steel",  title:"Server Analytics",             desc:"Track usage, uploads, questions asked, and overall bot activity from a dashboard." },
  { icon:"sync",           fill:1, color:"sage",   title:"Auto URL Updation",            desc:"Automatically monitor and sync URLs to refresh the knowledge base with the latest updates." },
  { icon:"image_search",   fill:1, color:"steel",  title:"Image & Screenshot OCR",       desc:"Upload screenshots or images and the bot extracts and indexes text from them automatically." },
  { icon:"translate",      fill:1, color:"sage",   title:"Auto Language Detection",      desc:"The bot automatically detects the user's language and replies in the same language every time." },
  { icon:"link",           fill:1, color:"steel",  title:"Source Citations",             desc:"Every answer references the exact document or section it came from." },
  { icon:"search",         fill:1, color:"sage",   title:"Web Search Fallback",          desc:"When your docs don't have the answer, the bot searches the web and labels the result." },
];

const colorMap = {
  steel: { bg:"rgba(89,131,146,0.12)",  border:"rgba(89,131,146,0.28)",  text:"#7fb3c3" },
  sage:  { bg:"rgba(174,195,176,0.1)",  border:"rgba(174,195,176,0.22)", text:"#AEC3B0" },
};

const STEPS = [
  { n:"01", color:"steel", icon:"shield_lock",  fill:1, title:"Login with Discord",    desc:"OAuth login verifies you're a server admin. No passwords, no extra signups." },
  { n:"02", color:"sage",  icon:"upload_file",  fill:1, title:"Upload Your Knowledge", desc:"Drag in PDFs, paste website URLs, or import Excel sheets. The crawler handles entire doc sites." },
  { n:"03", color:"steel", icon:"tune",         fill:1, title:"Tune & Configure",      desc:"Set which channels the bot watches, customize its persona, and dial in retrieval parameters." },
  { n:"04", color:"sage",  icon:"smart_toy",    fill:1, title:"Bot Goes Live",         desc:"Members ask questions in Discord and get precise answers pulled from your own documents." },
];
...
function SectionLabel({ text }) {
  return (
    <div style={{ display:"inline-flex",alignItems:"center",gap:8,padding:"5px 14px",borderRadius:99,background:"rgba(89,131,146,0.1)",border:"1px solid rgba(89,131,146,0.28)",marginBottom:16 }}>
...
function H2({ children }) {
  return (
    <h2 style={{ fontFamily:"'Cormorant Garamond',serif",fontWeight:400,fontSize:"clamp(28px,4vw,52px)",lineHeight:1.1,letterSpacing:"-0.01em",color:"var(--cream)",marginBottom:12 }}>{children}</h2>
  );
}

function Tick({ v }) {
  if (v === "yes") return <span style={{ display:"inline-flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:"50%",background:"rgba(89,131,146,0.18)",color:"var(--accent-light)",fontSize:14,fontWeight:700 }}>✓</span>;
  if (v === "no")  return <span style={{ display:"inline-flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:"50%",background:"rgba(255,255,255,0.04)",color:"#2d4a55",fontSize:14 }}>✗</span>;
  return                  <span style={{ display:"inline-flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:"50%",background:"rgba(174,195,176,0.1)",color:"var(--sage)",fontSize:14 }}>~</span>;
}
...
export default function LandingPage({ user, onLogin, onShowDashboard }) {
  ...
  return (
    <div ref={pageRef} style={{ background:"var(--abyss)",color:"var(--text)",minHeight:"100vh" }}>
      <style>{CSS}</style>
      ...
      <nav style={{
        ...
        padding:"0 64px",
        background: scrolled ? "rgba(1,22,30,0.96)" : "rgba(1,22,30,0.75)",
        backdropFilter:"blur(20px)",
        borderBottom:`1px solid ${scrolled ? "rgba(89,131,146,0.25)" : "var(--border)"}`,
        transition:"all 0.3s ease",
      }}>
        <a href="#" style={{ textDecoration:"none",display:"flex",alignItems:"center",gap:10 }}>
          <div style={{ width:30,height:30,borderRadius:8,background:"var(--navy)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 16px var(--steel-glow)",border:"1px solid rgba(89,131,146,0.4)" }}>
            <Icon name="shield_lock" size={16} fill={1} style={{ color:"var(--accent-light)" }} />
          </div>
          <span style={{ fontFamily:"'Cormorant Garamond',serif",fontWeight:400,fontSize:22,color:"var(--cream)",letterSpacing:"0.01em" }}>VaultBot</span>
        </a>
        <div className="hide900" style={{ display:"flex",alignItems:"center",gap:2 }}>
          ...
          {user ? (
            <button onClick={onShowDashboard} className="btn-sm" style={{ marginLeft:12,display:"flex",alignItems:"center",gap:8,padding:"8px 20px",borderRadius:8,fontSize:13,fontWeight:600,background:"var(--navy)",color:"var(--cream)",border:"1px solid rgba(89,131,146,0.4)",cursor:"pointer",transition:"all var(--tr)" }}>
              <Icon name="grid_view" size={15} /> Dashboard
            </button>
          ) : (
            <button onClick={onLogin} className="btn-sm" style={{ marginLeft:12,display:"flex",alignItems:"center",gap:8,padding:"8px 20px",borderRadius:8,fontSize:13,fontWeight:600,background:"var(--navy)",color:"var(--cream)",border:"1px solid rgba(89,131,146,0.4)",cursor:"pointer",transition:"all var(--tr)" }}>
      ...
      <section style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"0 64px",position:"relative",overflow:"hidden" }}>
        ...
        <div className="bloom-sage" style={{ bottom:"10%",right:"-5%",width:500,height:500 }} />
        ...
        <div className="hero-inner" style={{ width:"100%",maxWidth:1200,display:"flex",alignItems:"center",gap:72,paddingTop:64,position:"relative",zIndex:1 }}>
          ...
          <div style={{ flex:1,minWidth:0 }}>
            <div className="a0" style={{ display:"inline-flex",alignItems:"center",gap:8,padding:"6px 16px",borderRadius:99,background:"rgba(89,131,146,0.1)",border:"1px solid rgba(89,131,146,0.3)",fontSize:12,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",color:"var(--accent-light)",marginBottom:28 }}>
              <span className="ripple-dot" style={{ width:7,height:7,borderRadius:"50%",background:"var(--steel)",display:"inline-block" }} />
              RAG-Powered Discord AI
            </div>

            <h1 className="a1" style={{ fontFamily:"'Cormorant Garamond',serif",fontWeight:400,fontSize:"clamp(40px,5.5vw,76px)",lineHeight:1.04,letterSpacing:"-0.01em",color:"var(--cream)",marginBottom:22 }}>
            ...
            <div className="a3" style={{ display:"flex",gap:12,flexWrap:"wrap",marginBottom:40 }}>
              <button onClick={user ? onShowDashboard : onLogin} className="btn-primary" style={{ display:"flex",alignItems:"center",gap:10,padding:"14px 28px",borderRadius:10,fontSize:15,fontWeight:600,background:"var(--navy)",color:"var(--cream)",border:"1px solid rgba(89,131,146,0.5)",cursor:"pointer",boxShadow:"0 0 28px rgba(89,131,146,0.2)",transition:"all var(--tr)" }}>
                <DiscordIcon size={18} /> {user ? "Go to Dashboard" : "Invite to Discord"}
                <Icon name="arrow_forward" size={18} />
              </button>
              <button onClick={() => document.getElementById("playground")?.scrollIntoView({ behavior:"smooth" })} className="btn-ghost" style={{ display:"flex",alignItems:"center",gap:8,padding:"14px 22px",borderRadius:10,fontSize:15,fontWeight:500,background:"rgba(89,131,146,0.05)",color:"var(--muted)",border:"1px solid var(--border)",cursor:"pointer",transition:"all var(--tr)" }}>
                View Live Demo
              </button>
            </div>
          ...
          <div className="a5 hide900" style={{ width:380,flexShrink:0,position:"relative" }}>
            <div className="mascot-bounce" style={{ position:"absolute",top:-90,right:-30,zIndex:100,pointerEvents:"none",display:"flex",flexDirection:"column",alignItems:"center" }}>
              <div style={{ background:"linear-gradient(135deg,var(--navy),var(--navy-mid))",padding:"7px 13px",borderRadius:"12px 12px 0 12px",color:"var(--cream)",fontSize:10,fontWeight:700,boxShadow:"0 8px 20px rgba(18,69,89,0.5)",marginBottom:8,whiteSpace:"nowrap",border:"1px solid rgba(89,131,146,0.35)" }}>
                Hey! Try my live demo below! 🤖🌊
              </div>
              <svg width="85" height="85" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ filter:"drop-shadow(0 8px 20px rgba(89,131,146,0.4))" }}>
                <rect x="18" y="24" width="64" height="52" rx="20" fill="#071520" stroke="#598392" strokeWidth="3" />
                <rect x="23" y="29" width="54" height="42" rx="14" fill="#030f15" />
                <g className="mascot-antenna">
                  <line x1="50" y1="24" x2="50" y2="12" stroke="#598392" strokeWidth="3.5" strokeLinecap="round" />
                  <circle cx="50" cy="9" r="5" fill="#7fb3c3" />
                </g>
                <g className="mascot-eye">
                  <circle cx="38" cy="48" r="6" fill="#598392" />
                  <circle cx="38" cy="48" r="2.5" fill="var(--cream)" />
                  <path d="M 57 48 Q 62 44 67 48" stroke="#598392" strokeWidth="3" strokeLinecap="round" fill="none" />
                </g>
                <ellipse cx="32" cy="58" rx="3.5" ry="1.5" fill="#AEC3B0" opacity="0.45" />
                <ellipse cx="68" cy="58" rx="3.5" ry="1.5" fill="#AEC3B0" opacity="0.45" />
                <path d="M 46 56 Q 50 59 54 56" stroke="var(--cream)" strokeWidth="2.5" strokeLinecap="round" fill="none" />
                <rect x="12" y="42" width="6" height="16" rx="3" fill="#124559" />
                <rect x="82" y="42" width="6" height="16" rx="3" fill="#124559" />
              </svg>
            </div>

            <div style={{ display:"grid",gridTemplateColumns:"repeat(6,1fr)",gridTemplateRows:"repeat(6,1fr)",gap:14,height:480 }}>
              <div className="glass" style={{ gridColumn:"1/7",gridRow:"1/5",borderRadius:14,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.5)",border:"1px solid rgba(89,131,146,0.2)" }}>
                <div style={{ background:"#050f17",padding:"12px 16px",borderBottom:"1px solid rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                  <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                    <Icon name="tag" size={18} style={{ color:"var(--muted2)" }} />
                    <span style={{ fontWeight:700,fontSize:13,color:"var(--cream)" }}>general-support</span>
                  </div>
                ...
                <div style={{ padding:16,background:"#071520",height:"calc(100% - 45px)",display:"flex",flexDirection:"column",gap:14 }}>
                  <div style={{ display:"flex",gap:10 }}>
                    <div style={{ width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#124559,#598392)",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"var(--cream)",fontWeight:700 }}>U</div>
                    <div>
                  ...
                  <div style={{ display:"flex",gap:10,background:"rgba(89,131,146,0.07)",padding:12,borderRadius:8,border:"1px solid rgba(89,131,146,0.2)" }}>
                    <div style={{ width:36,height:36,borderRadius:"50%",background:"var(--navy)",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid rgba(89,131,146,0.4)" }}>
                      <Icon name="robot_2" size={18} fill={1} style={{ color:"var(--accent-light)" }} />
                    </div>
                    <div>
                      ...
                      <div style={{ color:"var(--text)",fontSize:13,lineHeight:1.6 }}>
                        Based on your docs, support is <strong style={{ color:"var(--cream)" }}>Mon–Fri, 9 AM – 5 PM</strong>.
                  ...
                  <div style={{ display:"flex",gap:10 }}>
                    <div style={{ width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#124559,#598392)",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"var(--cream)",fontWeight:700 }}>A</div>
              ...
              <div className="glass" style={{ gridColumn:"1/4",gridRow:"5/7",borderRadius:12,padding:"20px 18px",display:"flex",flexDirection:"column",justifyContent:"center",border:"1px solid rgba(89,131,146,0.18)" }}>
                <div style={{ fontFamily:"'Cormorant Garamond',serif",fontWeight:400,fontSize:32,color:"var(--accent-light)",lineHeight:1 }}>1.2ms</div>
              ...
              <div className="glass" style={{ gridColumn:"4/7",gridRow:"5/7",borderRadius:12,padding:"20px 18px",display:"flex",flexDirection:"column",justifyContent:"center",border:"1px solid rgba(89,131,146,0.18)" }}>
                <div style={{ fontFamily:"'Cormorant Garamond',serif",fontWeight:400,fontSize:32,color:"var(--sage)",lineHeight:1 }}>99.9%</div>
          ...
          <div style={{ display:"flex",gap:32,flexWrap:"wrap",alignItems:"flex-start" }}>
            <div style={{ flex:"1 1 340px",display:"flex",flexDirection:"column",gap:12 }}>
              ...
              {PLAYGROUND_PRESETS.map((p,idx) => (
                <button key={idx} onClick={() => handlePlaygroundRun(idx)} className="glass" style={{ display:"flex",flexDirection:"column",gap:8,padding:"16px 20px",borderRadius:12,border:`1.5px solid ${activePreset===idx?"var(--steel)":"var(--border)"}`,background:activePreset===idx?"rgba(89,131,146,0.07)":"rgba(255,255,255,0.02)",color:"var(--cream)",cursor:"pointer",textAlign:"left",outline:"none",transition:"all var(--tr)" }}>
              ...
              {selectedCitation && (
                <div className="glass" style={{ marginTop:16,padding:16,borderRadius:12,border:"1px solid rgba(89,131,146,0.3)",background:"rgba(89,131,146,0.05)" }}>
            ...
            <div className="glass" style={{ flex:"2 2 500px",borderRadius:16,overflow:"hidden",boxShadow:"0 20px 50px rgba(0,0,0,0.5)",border:"1px solid rgba(89,131,146,0.2)",background:"#050f17" }}>
              <div style={{ background:"#030b12",padding:"14px 20px",borderBottom:"1px solid rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                  <Icon name="tag" size={20} style={{ color:"var(--muted2)" }} />
                  <span style={{ fontWeight:700,fontSize:14,color:"var(--cream)" }}>#vaulty-playground</span>
                </div>
                <div style={{ fontSize:11,fontWeight:700,color:"var(--accent-light)",background:"rgba(89,131,146,0.14)",padding:"4px 10px",borderRadius:6,border:"1px solid rgba(89,131,146,0.28)" }}>
                  PLAYGROUND RETRIEVAL SIMULATOR
                </div>
              </div>

              <div style={{ padding:24,background:"#071520",minHeight:340,display:"flex",flexDirection:"column",gap:20 }}>
                {playgroundMessages.map((msg,i) => (
                  <div key={i} style={{ display:"flex",gap:14,animation:"fadeUp 0.3s ease both" }}>
                    {msg.sender==="user" ? (
                      <>
                        <div style={{ width:38,height:38,borderRadius:"50%",background:"linear-gradient(135deg,#124559,#598392)",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"var(--cream)",fontWeight:700 }}>U</div>
                        <div>
                          <div style={{ fontSize:11,color:"var(--muted2)",marginBottom:4 }}>User · simulated</div>
                          <div style={{ color:"var(--cream)",fontSize:14,lineHeight:1.5 }}>{msg.text}</div>
                        </div>
                      </>
                    ) : (
                      <>
                        <div style={{ width:38,height:38,borderRadius:"50%",background:"var(--navy)",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 12px var(--steel-glow)",border:"1px solid rgba(89,131,146,0.4)" }}>
                          <Icon name="robot_2" size={20} fill={1} style={{ color:"var(--accent-light)" }} />
                        </div>
                        <div style={{ flex:1 }}>
                          <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:4 }}>
                            <span style={{ fontWeight:700,fontSize:14,color:"var(--cream)" }}>Vaulty</span>
                            <span style={{ fontSize:9,fontWeight:800,textTransform:"uppercase",background:"var(--navy)",color:"var(--accent-light)",padding:"2px 6px",borderRadius:4,letterSpacing:"0.05em",border:"1px solid rgba(89,131,146,0.4)" }}>BOT</span>
                          ...
                          {msg.citations.length > 0 && (
                            <div style={{ display:"flex",flexWrap:"wrap",gap:8,marginTop:12 }}>
                              {msg.citations.map((c,j) => (
                                <button key={j} onClick={() => setSelectedCitation(c)} style={{ display:"flex",alignItems:"center",gap:6,background:"rgba(89,131,146,0.1)",border:"1px solid rgba(89,131,146,0.3)",borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:600,color:"var(--accent-light)",cursor:"pointer",outline:"none",transition:"all var(--tr)" }}>
                ...
                {playgroundTyping && (
                  <div style={{ display:"flex",gap:14,alignItems:"center" }}>
                    <div style={{ width:38,height:38,borderRadius:"50%",background:"var(--navy)",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid rgba(89,131,146,0.4)" }}>
                      <Icon name="robot_2" size={20} fill={1} style={{ color:"var(--accent-light)" }} />
                    </div>
        ...
        <div className="mq" style={{ display:"flex",gap:44,alignItems:"center",whiteSpace:"nowrap" }}>
          {[...MQ_ITEMS,...MQ_ITEMS].map((m,i) => (
            <div key={i} style={{ display:"flex",alignItems:"center",gap:10,fontSize:13,fontWeight:500,color:"var(--muted)",flexShrink:0 }}>
              <div style={{ width:28,height:28,borderRadius:7,background:"var(--steel-dim)",border:"1px solid rgba(89,131,146,0.2)",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--accent-light)" }}>
      ...
      <div style={{ borderBottom:"1px solid var(--border)",background:"var(--s2)" }}>
        <div className="stats-grid" style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",maxWidth:1100,margin:"0 auto" }}>
          {[
            ...
            { val:"5",  suf:"+", label:"File Types Supported",    col:"var(--sage)" },
            { val:"∞",  suf:"",  label:"Servers You Can Add",     col:"var(--cream)" },
            { val:"90", suf:"%", label:"Cheaper Than Other Bots", col:"#7fb3c3" },
          ].map((s,i) => (
            <div key={i} style={{ padding:"36px 24px",textAlign:"center",borderRight:"1px solid var(--border)" }}>
              <div style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:52,fontWeight:400,lineHeight:1,marginBottom:8 }}>
                ...
                <span style={{ color:"var(--cream)" }}>{s.suf}</span>
              </div>
      ...
      <section id="howitworks" style={{ padding:"96px 64px",maxWidth:1200,margin:"0 auto" }}>
        ...
        <div className="steps-grid" style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:0,position:"relative" }}>
          <div style={{ position:"absolute",top:44,left:"12.5%",right:"12.5%",height:1,background:"linear-gradient(90deg,transparent,rgba(89,131,146,0.2),rgba(89,131,146,0.2),transparent)",pointerEvents:"none" }} />
          {STEPS.map((s,i) => {
            ...
            return (
              <div key={i} className="rv step-wrap" style={{ padding:"0 20px",display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",position:"relative",zIndex:1,transitionDelay:`${i*0.08}s` }}>
                <div className="step-icon" style={{ width:88,height:88,borderRadius:20,background:"var(--s2)",border:"1px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:20,position:"relative",transition:"all 0.3s ease",color:c.text }}>
                  ...
                  <span style={{ position:"absolute",top:-10,right:-10,width:26,height:26,borderRadius:"50%",background:s.color==="steel"?"var(--navy)":c.bg,border:`1px solid ${c.border}`,color:s.color==="steel"?"var(--accent-light)":c.text,fontSize:11,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif" }}>{s.n}</span>
                </div>
                {i < 3 && <span style={{ position:"absolute",right:-8,top:30,color:"var(--muted2)",fontSize:20 }}>→</span>}
                <div style={{ fontSize:14,fontWeight:600,color:"var(--cream)",marginBottom:8,fontFamily:"'DM Sans',sans-serif" }}>{s.title}</div>
      ...
      <section id="features" style={{ padding:"96px 64px",background:"var(--s2)" }}>
        <div style={{ maxWidth:1200,margin:"0 auto" }}>
          ...
          <div className="rv glass glass-hover feat-card" style={{ borderRadius:16,padding:"32px",marginBottom:20,display:"flex",flexDirection:"column",gap:20,position:"relative",overflow:"hidden",border:"1px solid rgba(89,131,146,0.18)" }}>
            <div className="feat-icon" style={{ width:52,height:52,borderRadius:14,background:"rgba(89,131,146,0.14)",border:"1px solid rgba(89,131,146,0.3)",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--accent-light)",transition:"all 0.3s ease" }}>
              <Icon name="dashboard_customize" size={26} fill={1} />
            </div>
            <h3 style={{ fontFamily:"'Cormorant Garamond',serif",fontWeight:400,fontSize:30,color:"var(--cream)",letterSpacing:"-0.01em" }}>Live Admin Control & Auto-Sync Engine</h3>
            ...
            <div style={{ display:"flex",gap:10,flexWrap:"wrap" }}>
              {["Drag & Drop Ingestion","Auto-Sync Scheduler","Visual Analytics Panel","Zero-Code Setup","Per-Channel Controls"].map((t,i) => (
                <span key={i} style={{ background:"rgba(89,131,146,0.07)",border:"1px solid var(--border)",padding:"5px 12px",borderRadius:6,fontSize:12,fontWeight:500,color:"var(--muted)",letterSpacing:"0.02em" }}>{t}</span>
              ))}
            </div>
          ...
          <div className="feats-grid" style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:1,background:"rgba(89,131,146,0.06)",border:"1px solid rgba(89,131,146,0.06)",borderRadius:16,overflow:"hidden" }}>
            {FEATS.map((f,i) => {
              ...
              return (
                <div key={i} className="rv feat-card" style={{ background:"var(--s2)",padding:24,transitionDelay:`${(i%3)*0.04}s` }}>
                  ...
                  <div style={{ fontSize:14,fontWeight:600,color:"var(--cream)",marginBottom:6 }}>{f.title}</div>
          ...
          <div className="use-grid" style={{ display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:20 }}>
            {USE_CASES.map((u,i) => (
              <div key={i} className="rv use-card glass" style={{ padding:32,borderRadius:16,transition:"all 0.25s ease",transitionDelay:`${i*0.05}s` }}>
                <div style={{ width:50,height:50,borderRadius:14,background:"var(--steel-dim)",border:"1px solid rgba(89,131,146,0.25)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:18,color:"var(--accent-light)" }}>
                  <Icon name={u.icon} size={24} fill={u.fill} />
                </div>
                <span style={{ display:"inline-block",padding:"3px 10px",borderRadius:99,fontSize:11,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",background:"var(--steel-dim)",color:"var(--accent-light)",border:"1px solid rgba(89,131,146,0.25)",marginBottom:12 }}>{u.tag}</span>
                <div style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:400,color:"var(--cream)",marginBottom:10 }}>{u.title}</div>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:14,marginTop:8 }}>
                {col.items.map(([b,r],j) => (
                  <div key={j} style={{ display:"flex",alignItems:"flex-start",gap:12 }}>
                    <div style={{ width:22,height:22,borderRadius:"50%",background:"var(--steel-dim)",border:"1px solid rgba(89,131,146,0.25)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:2,color:"var(--accent-light)" }}>
                      <Icon name="check" size={12} />
                    </div>
                    <span style={{ fontSize:15,color:"var(--muted)",lineHeight:1.6,fontWeight:300 }}>
                      <strong style={{ color:ci===0?"var(--cream)":"var(--accent-light)",fontWeight:600 }}>{b}</strong> {r}
                    </span>
                  </div>
                ))}
              </div>
      ...
      <section id="compare" style={{ padding:"96px 64px" }}>
        <div style={{ maxWidth:1200,margin:"0 auto" }}>
          ...
          <div className="rv glass" style={{ borderRadius:16,overflow:"hidden",border:"1px solid rgba(89,131,146,0.15)" }}>
              ...
              <thead>
                <tr style={{ background:"var(--s3)" }}>
                  ...
                  {["VaultBot AI","Other Bots","What's Missing?"].map((h,i) => (
                    <th key={i} style={{ padding:"16px 24px",fontWeight:700,fontSize:11,letterSpacing:"0.06em",textTransform:"uppercase",borderBottom:i===0?"2px solid var(--steel)":"1px solid var(--border)",textAlign:i===2?"left":"center",color:i===0?"var(--accent-light)":"var(--muted2)",background:i===0?"rgba(89,131,146,0.07)":"transparent" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {COMP.map((row,i) => (
                  <tr key={i} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding:"14px 24px",textAlign:"center",background:"rgba(89,131,146,0.03)" }}><Tick v={row[1]} /></td>
      ...
      <section id="faq" style={{ padding:"96px 64px",background:"var(--s2)" }}>
        <div style={{ maxWidth:760,margin:"0 auto",textAlign:"center" }}>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:10,marginTop:44,textAlign:"left" }}>
            {FAQS.map((f,i) => (
              <div key={i} className="rv" onClick={() => setFaq(faq===i?null:i)} style={{ background:"var(--s3)",border:`1px solid ${faq===i?"rgba(89,131,146,0.35)":"var(--border)"}`,borderRadius:13,overflow:"hidden",cursor:"pointer",transition:"border-color var(--tr)",transitionDelay:`${i*0.04}s` }}>
                <div style={{ padding:"18px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,fontSize:15,fontWeight:600,color:faq===i?"var(--accent-light)":"var(--cream)",fontFamily:"'DM Sans',sans-serif" }}>
                  {f.q}
                  <span style={{ width:28,height:28,borderRadius:8,background:faq===i?"var(--steel-dim)":"rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"center",color:faq===i?"var(--accent-light)":"var(--muted2)",flexShrink:0,transition:"transform var(--tr),background var(--tr)",transform:faq===i?"rotate(180deg)":"none" }}>
      ...
      <section style={{ padding:"110px 64px",textAlign:"center",position:"relative",overflow:"hidden" }}>
        ...
        <div className="bloom-sage" style={{ top:"30%",right:"10%",width:400,height:400 }} />
        <div style={{ position:"relative",zIndex:1 }}>
          <div className="rv glass" style={{ maxWidth:860,margin:"0 auto",borderRadius:28,padding:"80px 60px",display:"inline-block",width:"100%",border:"1px solid rgba(89,131,146,0.2)" }}>
            <div className="rv"><SectionLabel text="Get Started" /></div>
            <h2 className="rv" style={{ fontFamily:"'Cormorant Garamond',serif",fontWeight:400,fontSize:"clamp(30px,5vw,60px)",color:"var(--cream)",letterSpacing:"-0.01em",lineHeight:1.08,marginBottom:16 }}>Ready to automate your support?</h2>
            ...
            <div className="rv" style={{ display:"flex",justifyContent:"center",gap:14,flexWrap:"wrap" }}>
              <button onClick={onLogin} className="btn-primary glow-ocean" style={{ display:"flex",alignItems:"center",gap:10,padding:"16px 32px",borderRadius:14,fontSize:16,fontWeight:600,background:"var(--navy)",color:"var(--cream)",border:"1px solid rgba(89,131,146,0.5)",cursor:"pointer",boxShadow:"0 0 28px var(--steel-glow)",transition:"all var(--tr)" }}>
                <DiscordIcon size={18} /> Add VaultBot to Discord
              </button>
              <a href="#features" className="btn-ghost" style={{ display:"flex",alignItems:"center",gap:8,padding:"16px 26px",borderRadius:14,fontSize:16,fontWeight:500,background:"rgba(89,131,146,0.05)",color:"var(--muted)",border:"1px solid var(--border)",textDecoration:"none",transition:"all var(--tr)" }}>
                Browse Features →
              </a>
            </div>
        ...
        <div style={{ maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:32 }}>
          <div>
            <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:16 }}>
              <div style={{ width:30,height:30,borderRadius:8,background:"var(--navy)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 14px var(--steel-glow)",border:"1px solid rgba(89,131,146,0.4)" }}>
                <Icon name="shield_lock" size={15} fill={1} style={{ color:"var(--accent-light)" }} />
              </div>
              <span style={{ fontFamily:"'Cormorant Garamond',serif",fontWeight:400,fontSize:22,color:"var(--cream)" }}>VaultBot</span>
            </div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:24 }}>
            <div>
              <h4 style={{ fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:700,color:"var(--cream)",marginBottom:16,letterSpacing:"0.06em",textTransform:"uppercase" }}>Supported Ingestion</h4>
            ...
            <div>
              <h4 style={{ fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:700,color:"var(--cream)",marginBottom:16,letterSpacing:"0.06em",textTransform:"uppercase" }}>Built With</h4>
              <ul style={{ display:"flex",flexDirection:"column",gap:10,listStyle:"none",padding:0 }}>
